...
CI (GitHub Actions):
- A workflow is provided at .github/workflows/smoke-tests.yml. On pull_request it installs backend deps, prepares the DB, starts the backend, then runs the cart smoke test (npm run smoke:cart). The job stops the server afterward.
